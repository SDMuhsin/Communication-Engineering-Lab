...
for SNR_index = 1:1:length(SNR)
    ...
    qam3ErrorSim(SNR_index) = 7*qfunc(sqrt(snr/2));
end
figure(4)
semilogy(SNR,qam1ErrorSim,'LineWidth',1.5)
hold on
...
xlabel('E_{s}/No (dB)')
ylabel('Symbol Error')
title (' QAM symbol Error')
legend('Rectangular QAM ','Circular QAM 1','Circular QAM 2')
grid on 
